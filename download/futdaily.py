...
        ----------
    ...
        ----------
        ...
        trade_date_lst1 = pd.read_sql(sql=sql1,
                                      con=self.engine)['trade_date'].tolist()
...
        ----------
 ...
        ----------
    ...
    def download_dailyprices(self):
        ...
        for trade_date in tqdm(self.trade_date_lst):
            ...
            fields_lst = [
                ...
                'settle', 'pre_close', 'pre_settle', 'vol', 'amount', 'oi',
                'delv_settle'
            ]
            fields = ','.join(fields_lst)
            df = downloader.download(pro.fut_daily,
    ...
    def download_futbasic(self):
        # 拉取数据
        fields = [
            'ts_code', 'exchange', 'name', 'multiplier', 'trade_unit',
        ...
        for exchange in exchange_lst:
            tempdf = downloader.download(pro.fut_basic,
                                         exchange=exchange,
                                         fields=fields)
            tempdf = tempdf.rename(columns={'ts_code': 'fut_code'})
            tempdf = self._select_trading_contract(tempdf)
            tempdf['is_after_hours_trading'] = \
                tempdf['trade_time_desc'].apply(lambda x: 1 if '夜盘' in x else 0)
    ...
    def download_futwsr(self):
        ...
        for trade_date in tqdm(self.trade_date_lst):
            # 日频数据
            fields_lst = [
                'trade_date', 'symbol', 'exchange', 'warehouse', 'vol',
                'pre_vol', 'area', 'year', 'unit'
            ]
            fields = ','.join(fields_lst)
            df = downloader.download(pro.fut_wsr,
                                     trade_date=trade_date,
                                     fields=fields)
    ...
    def download_futholding(self):
        ...
        for exchange in exchange_lst:
            ...
            for trade_date in tqdm(self.trade_date_lst):
                ...
                fields = ','.join(fields_lst)
                df = downloader.download(pro.fut_holding,
                                         trade_date=trade_date,
                                         exchange=exchange,
                                         fields=fields)
                sql_dtype = {
