...
        ----------
    ...
    def download_dailyprices(self):
        ...
        for trade_date in tqdm(self.trade_date_lst):
            ...
            df1 = downloader.download(pro.daily,
    ...
    def download_dailybasic(self):
        ...
        for trade_date in tqdm(self.trade_date_lst):
            ...
            fields = ','.join(fields_lst)
            df = downloader.download(pro.daily_basic,
                                     trade_date=trade_date,
                                     fields=fields)
            df = df.rename(columns={'ts_code': 'stock_code'})
            sql_dtype = {
    ...
    def download_stockbasic(self):
        # 拉取数据
        fields = [
            "ts_code", "name", "area", "market", "exchange", "list_date",
            "delist_date"
        ]
        df = pd.DataFrame()
        for list_status in ['L', 'D', 'P']:
            tempdf = downloader.download(pro.stock_basic,
