...
class TableStructure(DataBase):
    ...
    def download_table_structure(self):
        '''
        Description
        ----------
        下载表结构
    ...
    @logger_decorator(logger)
    def download_table_comment(self):
        '''
        Description
        ----------
        下载表注释
    ...
    @logger_decorator(logger)
    def download_all_structure(self,
                               rename_old_table=True,
                               remove_past_days=7):
        '''
        Description
        ----------
     ...
        ----------
        ...
        remove_past_days: int. 需要去除多少天前的表格结构. 默认为7.
            如果输入为0，则不去除历史表格结构
     ...
        ----------
        ...
        # 删除remove_past_days之前的表格
        if remove_past_days > 0:
            try:
                self._remove_history_structure_df(remove_past_days=remove_past_days)
            except Exception as e:
                raise FileExistsError(e)
        tb_struct_df = self.download_table_structure()
        tb_ind_df = self.download_table_index()
        tb_comm_df = self.download_table_comment()
 ...
        ----------
    ...
    @logger_decorator(logger)
    def _remove_history_structure_df(self, remove_past_days=7):
        '''
        Description
        ----------
        删除历史remove_past_days之前的表结构

        Parameters
        ----------
        remove_past_days: int. 多少天之前需要删除的表结构
 ...
        ----------
        ...
        for filename in os.listdir(dir_path):
            ...
            if (today_datetime - file_datetime).days >= remove_past_days:
